...
const parseItemDescriptionWithGPT = async (description) => {
  try {
    const response = await openai.createChatCompletion({
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: "You are a helpful assistant that parses item descriptions into mods and item names. Return the result in JSON format like this: { \"mods\": [\"mod1\", \"mod2\"], \"item\": \"item_name\" }" },
        { role: "user", content: `Parse the following item description into its components (mods and item): "${description}".` },
      ],
      temperature: 0.5,
      max_tokens: 64,
      top_p: 1,
