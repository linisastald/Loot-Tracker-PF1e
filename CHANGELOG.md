...
### Added
- **"Level Up" button for the party (DM only).** On the DM Campaign Settings page, a new Party Level section shows the campaign's current average party level and a Level Up button that raises it by one. If Discord integration is enabled, it posts an announcement to the configured channel (tagging the campaign role) telling players which level to level up to. A confirmation dialog guards the action since it pings Discord.
